#!/bin/bash
# Description: Combined utility for running official Ookla speed tests and local network/port audits with interactive prompts.
set -eo pipefail
...
read -p "Would you like to run the Ookla Speed Test? [y/N]: " RUN_SPEEDTEST </dev/tty
RUN_SPEEDTEST=${RUN_SPEEDTEST:-N}

read -p "Would you like to run the Local Network & Port Audit? [y/N]: " RUN_AUDIT </dev/tty
RUN_AUDIT=${RUN_AUDIT:-N}
...
if [[ "$RUN_AUDIT" =~ ^[yY]$ ]]; then
    ...
    echo ""
    echo "📡 Pi Network Interface & Gateway:"
    ...
    echo "🛡️ Raspberry Pi Open Listening Ports:"
